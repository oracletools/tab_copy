...
class SimpleQueryPanel(wx.Panel):
	...
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent,panel,ignore_change=False):
		""""""
...
class SimpleResultsNbPanel(wx.Panel):
	...
	def __init__(self, parent,panel):
		...
		nb.SetTabAreaColour(wx.Color(222,222,222))
		# add some pages to the notebook
		self.pages = [(ResultsPanel(nb), "Results"),
				 (OutputPanel(nb), "Output") #,
				 #(ExportPanel(nb), "Export")
...
class SimpleTabPanel(wx.Panel):
	...
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent, nb, ignore_change=False):
		""""""
		...
		self.qp=SimpleQueryPanel(self.splitter,self, ignore_change)
...
class SQLPanel(wx.Panel):
	...
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent, log, file_to_open=None):
		""""""
		wx.Panel.__init__(self, parent=parent, id=wx.ID_ANY)
...
class SimpleSQLPanel(wx.Panel):
	...
	def __init__(self, parent, log, file_to_open=None):
		...
		# create the AuiNotebook instance
		#print dir(fnb)
		self.nb = fnb.FlatNotebook(self, -1,size=(700,500), agwStyle=fnb.FNB_NO_X_BUTTON|fnb.FNB_COLOURFUL_TABS )
	...
	def AddNew(self, label='', value='', ignore_change=False):
		...
		page = SimpleTabPanel(self,self.nb,ignore_change)
		#print(dir(page))
		print 'page.Id= ', page.Id
		page.file_name=label
		page.saved=False
...
class SimpleSQLEditor(wx.MiniFrame):
	def __init__(self, parent, log, title, file_to_open):
		#wx.Frame.__init__(self, parent, -1, title, size=(840,680))
		wx.MiniFrame.__init__(self, parent, -1, title,  pos=wx.DefaultPosition, size=(940,780),  style =wx.CAPTION|wx.SYSTEM_MENU | wx.FRAME_FLOAT_ON_PARENT|wx.THICK_FRAME)
		self.panel = SimpleSQLPanel(self, log, file_to_open)
		#self._mgr = AUIManager(self)
		self.finddata = wx.FindReplaceData()
		self.finddata.SetFlags(wx.FR_DOWN)
		#self.firstTime = True
