...
class TableCopyTreeList(HTL.HyperTreeList): #,TextEditMixin):
	def __init__(self, parent, form, id=wx.ID_ANY, pos=wx.DefaultPosition,
		...
		if 1:
			...
			task = self.AppendItem(worker_, 'task "Table Copy"')
			#self.tree.SetItemText(wk, 'Template: Query Copy', 1)
			...
			self.SetPyData(task,wx.TreeItemData(('exec_copy',{})))
				...
				for w in r.GetChildren():
					...
					if w_tag in ('worker'):
						...
						for t in w.GetChildren():
							...
							t_utils=doc_to.createElement('sqlp')
...
#--------------------------------------------------------------------------------------------------		
class TCD_Configuration(wx.Panel):
	...
	def __init__(self, parent,style,data,pos,spath,tpath, source):
		...
		self.tree = TableCopyTreeList(self.splitter,self, -1 #, #style =
										#wx.TR_DEFAULT_STYLE
										#| wx.TR_HAS_BUTTONS
										#| wx.TR_TWIST_BUTTONS
										#| wx.TR_ROW_LINES
...
class TableCopyPanel(wx.Panel):
	def __init__(self, parent,pos,drag_pos, drop_pos, dd_data, table_name ):
		...
		#sys.exit(0)
		self.config_panel=TCD_Configuration(self,style=wx.TAB_TRAVERSAL|wx.CLIP_CHILDREN, data=self.data,pos=pos, spath=self.cfrom, tpath=self.cto, source=(self.schema_from,self.table_from))
