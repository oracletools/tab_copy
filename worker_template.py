...
class worker_Template:
	...
	def setWorker(self, worker):
		...
		for n in worker.childNodes:
			if n.nodeType==worker.ELEMENT_NODE:
				...
				cdata = self.tree.AppendItem(task, cd)
	...
	def getCDATA(self,worker,util_node):
		return 'Template ="%s"' %[n for n in util_node.getElementsByTagName("sql_template")[0].childNodes if n.nodeType==worker.CDATA_SECTION_NODE][0].data
	def CreateWorkerTree(self):
		...
		self.pipeline = self.tree.AppendItem(self.ppln.root, ppl_type)
		...
		self.tree.Expand(self.ppln.root)
		
class worker_TableCopy(worker_Template):
	def	__init__(self, pipeline):
		worker_Template.__init__(self, pipeline)
		self.template_name='trunc_comp_stats_sharded_table_copy.xml'
		#self.tree=tree	
	def setWorkerParams(self):
		self.p={} #params
	...
	def getValue(self,n):
		...
		print  value
		return value
class worker_PartCopy(worker_Template):
	def	__init__(self, pipeline):
		worker_Template.__init__(self, pipeline)
		self.template_name='trunc_comp_stats_sharded_part_copy.xml'
		#self.tree=tree	
	def setWorkerParams(self):
		self.p={} #params
		self.p['db_from'] =self.form.spath.split('/')[3]
	...
	def getValue(self,n):
		...
		print  value
		return value		
class worker_SubPartCopy(worker_Template):
	def	__init__(self, pipeline):
		worker_Template.__init__(self, pipeline)
		self.template_name='trunc_comp_stats_sharded_spart_copy.xml'
		#self.tree=tree	
	def setWorkerParams(self):
		self.p={} #params
		self.p['db_from'] =self.form.spath.split('/')[3]
	...
	def getValue(self,n):
		...
		print  value
		return value
class worker_QueryCopy(worker_Template):
	def	__init__(self, pipeline):
		worker_Template.__init__(self, pipeline)
		self.template_name='query_copy.xml'
		#self.tree=tree	
	def setWorkerParams(self):
		self.p={} #params
		self.p['db_from'] =self.form.spath.split('/')[3]
	...
	def getCDATA(self,worker,util_node):
		return 'Query = "%s"' % self.createQuery(self.form.data,self.form.spath)