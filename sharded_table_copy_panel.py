	...
	def run(self):
		...
		if status:	
			if  not self.is_trial:
				Publisher().sendMessage( "query_copy_done",  (status,err) )
			else:
			...
			if  not self.is_trial:
				Publisher().sendMessage( "append_log", ('Executing query copy...',self.obj.pos) )
				...
				(out,err)=cu.execTaCo(config_loc,worker_loc)
				Publisher().sendMessage( "query_copy_done",  (out,err) )
			else:
		...
		Publisher().sendMessage( "append_log", ('Done',self.obj.pos))
...
class ShardedTableCopyTreeList(HTL.HyperTreeList): #,TextEditMixin):
	def __init__(self, parent, form, id=wx.ID_ANY, pos=wx.DefaultPosition,
		...
		#sys.exit(1)
		tmpl_loc=os.path.join(appLoc,'xml_templates')
		template_name='sharded_table_copy.xml'
		tmpl_file_loc= os.path.join(tmpl_loc,template_name)
		doc = xml.dom.minidom.parseString(open(tmpl_file_loc, "r").read())
		tmpl=doc.getElementsByTagName("etldataflow")[0]
		tmpl_name=tmpl.getAttribute('name')
		self.ppl_name= tmpl_name+'_%s_%s' % self.source
		...
		if 1:
			...
			exec_node= [n for n in worker.childNodes if n.nodeType==worker.ELEMENT_NODE][0]
			print exec_node
			...
			worker_ = self.AppendItem(pipeline, 'WORKER "%s.%s"' % self.source)
			...
			task = self.AppendItem(worker_, 'task "Sharded Table Copy"')
			#self.tree.SetItemText(wk, 'Template: Query Copy', 1)
			...
			for n in tasklet.getElementsByTagName("param"):
				...
				if n.getAttribute('name')=='TO_TABLE':
					value=self.source[1]
				if n.getAttribute('name')=='TO_SCHEMA':
					value=self.source[0]					
				print  value
			...
			cdata = self.AppendItem(task, 'schema.table = "%s"' % self.schema_table)
			
			#self.tree.SetItemText(query, '%s...' % q[:15], 1)
			
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.SetPyData(cdata,wx.TreeItemData(('cdata',{'tag':'sql_template','cdata':self.schema_table})))
			self.Expand(task)
			self.Expand(worker_)
				...
				for w in r.GetChildren():
					...
					if w_tag in ('worker'):
						...
						for t in w.GetChildren():
							...
							t_utils.setAttribute('utils','%COPY_METHOD%' )
...
#--------------------------------------------------------------------------------------------------		
class STCD_Configuration(wx.Panel):
	"""Panel for copy config"""
	def __init__(self, parent,style,data,pos,spath,tpath, source):
		wx.Panel.__init__(self, parent, -1, style=style)
		self.data=data
		self.parentFrame=parent.frame
		self.pos=pos
		...
		self.tree = ShardedTableCopyTreeList(self.splitter,self, -1 #, #style =
										#wx.TR_DEFAULT_STYLE
										#| wx.TR_HAS_BUTTONS
										#| wx.TR_TWIST_BUTTONS
										#| wx.TR_ROW_LINES
		...
		if 1:
			...
			if 1:
				idx=1
				self.log = UltTacoLogger(self,self.pos) 
...
class ShardedTableCopyPanel(wx.Panel):
	def __init__(self, parent,pos,drag_pos, drop_pos, dd_data, table_name ):
		...
		#sys.exit(0)
		self.config_panel=STCD_Configuration(self,style=wx.TAB_TRAVERSAL|wx.CLIP_CHILDREN, data=self.data,pos=pos, spath=self.cfrom, tpath=self.cto, source=(self.schema_from,self.table_from))
	...
	def OnDeployXml(self,e):
		...
		if 1:
			self.worker = ExecQcThread((self), is_trial=True)
			self.worker.start()
	...
	def OnStart(self,e):
		...
		if 1:
			self.worker = ExecQcThread((self), is_trial=False)
