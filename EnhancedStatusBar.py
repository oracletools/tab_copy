...
class EnhancedStatusBar(wx.StatusBar):
	...
	def OnSize(self, event):
		...
		for pos, item_list in self._items.items():
	...
	def AddWidget(self, widget, horizontalalignment=ESB_ALIGN_CENTER_HORIZONTAL,
		...
		if not self._items.has_key(pos):
			self._items[pos]=[]
		self._items[pos].append(EnhancedStatusBarItem(widget, pos, horizontalalignment, verticalalignment))
		
		wx.CallAfter(self.OnSize, None)

	def AddWidget0(self, widget_dict, id, horizontalalignment=ESB_ALIGN_CENTER_HORIZONTAL,
				  verticalalignment=ESB_ALIGN_CENTER_VERTICAL, pos = -1):
		"""Add A Widget To The EnhancedStatusBar.
