...
#ddb='MRR_BI'
#duser='MRR_ETL_USER'
#from db_utils import query,printerr

imitateOracle=False
if imitateOracle:
	from  imitate_db_utils import query,printerr
else:
	from  db_utils import query,printerr
...
try:
	...
	class CodeEditor(SqlSTC):
		def __init__(self, parent, ID,readonly,style=wx.BORDER_NONE):
...
class SimpleGrid(gridlib.Grid): ##, mixins.GridAutoEditMixin):
	...
	def UpdateLimitedGrid(self, out,err, headers):
		#global d
					...
					for rs in out:
						...
						if 1:
							...
							for c in range(cols):			
								...
								print c
								print 'rs[c]', rs[c]
								print self.SetCellValue(i-1, c, str(rs[c]))
								self.Refresh()
							#headers=cur.description
							for d in range(len(headers)):
								#print headers[d]
		...
		print 'i=',i, rowcount
		return ( err, rowcount,headers)

	def UpdateLimitedGrid_prelib(self, sql,connect_as,ID):
		#global d
		(db, user) = connect_as
...
class QueryPanel(wx.Panel):
	...
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent,source,ignore_change=False):
		""""""
		...
		bsizer.Add(self.connect_db, 0, wx.RIGHT)
		bsizer.Add(self.connect_user, 0, wx.RIGHT)
		bsizer.Add(b, 0, wx.RIGHT)

		
		sizer = wx.BoxSizer(wx.VERTICAL)
		#sizer.Add(txtOne, 0, wx.ALL, 5)
		#sizer.Add(txtTwo, 0, wx.ALL, 5)
		sizer.Add((5,5), 1, wx.ALL)
		sizer.Add(bsizer, 0, wx.RIGHT, 1)
		print 'qp parent id=', parent.Id
		self.editor = CodeEditor(self, abs(parent.Id),readonly=True)
...
group by objname\n""" %(desc,tn)
		...
		if status and err:
			...
			if not status and headers:
				if rowcount>0:
					if 1:
						print 'posting'
						#Publisher().sendMessage( "add_describe", (td,desc) )
	...
	def ExecSQL(self, sql, connect_as=None):
		...
		(status, err, rowcount,headers, out)=query( sql, connect_as)		
		(err, rowcount, headers)=self.parent.rp.pages[0][0].grid.UpdateLimitedGrid(out,err,headers)
		#UpdateLimitedGrid(self, out,err, headers):
		print 'out= %s,%s,%s,%s ' % (status, err, rowcount, headers)
		if status and err: 
			self.parent.rp.pages[1][0].Error(err)
			#self.parent.rp.pages[1][0].SetFocus()
			...
			#ID_SAVE_AND_EXIT
			#Publisher().sendMessage( "enable_template_save", (self.ID) )		
			send("enable_template_save", (self.ID) )
	...
	def ExplainSQL(self,sql,db):
		...
		if status and err: 
			...
			if not status and headers:
				if rowcount>0:
					...
					#self.parent.rp.pages[0][0].SetFocus()
					#self.SetFocus()
					#Publisher().sendMessage( "add_explain_sql", ('explain',sql,db,exp) )
		...
		#print status, err
class SimpleQueryPanel(wx.Panel):
	"""
	This will be the first notebook tab
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent,panel,from_conn,ignore_change=False):
		""""""
		...
		users = (from_conn[0],)
		self.connect_db=wx.ComboBox(self, 0,   choices=dbs, style=wx.CB_READONLY)
		self.connect_db.SetValue(dbs[0])
		self.connect_user=wx.ComboBox(self, 0,   choices=users, style=wx.CB_READONLY)
		self.connect_user.SetValue(users[0])
		
		bsizer = wx.BoxSizer(wx.HORIZONTAL)
		bsizer.Add((5,5), 1, wx.ALL|wx.EXPAND)
		bsizer.Add(self.connect_user, 0, wx.RIGHT)
		bsizer.Add(self.connect_db, 0, wx.RIGHT)
		
		bsizer.Add(b, 0, wx.RIGHT)
		self.connect_user.Enable(False)
		self.connect_db.Enable(False)
...
group by objname\n""" %(desc,tn)
	...
	def ExecSQL(self, sql, connect_as=None):
		assert sql, 'sql statement is not defined'
		print 'ExecSQL',connect_as
		if not connect_as:
			connect_as=self.connect_as()
		self.parent.rp.sql=sql
		self.parent.rp.db=connect_as
		start_time = time.time()
		#(status, err, rowcount, headers)=self.parent.rp.pages[0][0].grid.UpdateLimitedGrid(sql,connect_as,self.parent.ID)
		...
		if 1:
			...
			try:
				...
				result = run_function_as_process_in_dialog(
					parent=self,
					thread_function=query,
					title="Query test.",
					message="Running query test process...",
					args=(sql, connect_as),
				...
				print '>>>>>>>>>>>',result
				print result
				#plog.log('pui_rcopyFile completed 3', self.ID)
...
class ResultsPanel(wx.Panel):
	...
	def onTextKeyEvent(self, event):
		...
		if keycode==344: #F5 
			...
			#Publisher().sendMessage( "re_exec_sql", (sql,db) )
...
########################################################################		
class SimpleResultsNbPanel(wx.Panel):
	"""
	This will be the first notebook tab
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent,panel):
		...
		# create the AuiNotebook instance
		#nb = wx.aui.AuiNotebook(self)
		nb = fnb.FlatNotebook(self, -1, agwStyle=fnb.FNB_SMART_TABS|fnb.FNB_DROPDOWN_TABS_LIST|fnb.FNB_NO_X_BUTTON|fnb.FNB_COLOURFUL_TABS )
		#nb.SetBackgroundColour((0, 0, 0))
		nb.SetTabAreaColour(wx.Colour(222,222,222))
		# add some pages to the notebook
		self.pages = [(ResultsPanel(nb), "Results"),
				 (OutputPanel(nb), "Output") #,
				 #(ExportPanel(nb), "Export")
...
class TabPanel(wx.Panel):
	...
	def __init__(self, parent,source, ignore_change=False):
		...
		#					wx.NO_BORDER | wx.TE_MULTILINE)
		self.qp=QueryPanel(self, source,ignore_change)
...
class ProportionalSplitter(wx.SplitterWindow):
	def __init__(self,parent, id = -1, proportion=0.8, size = wx.DefaultSize, **kwargs):
		wx.SplitterWindow.__init__(self,parent,id,wx.Point(0, 0),size, **kwargs)
		self.SetMinimumPaneSize(0) #the minimum size of a pane.
...
########################################################################
class SimpleTabPanel(wx.Panel):
	"""
	This will be the first notebook tab
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent, nb, from_conn,ignore_change=False):
		""""""
		self.ID=parent.ID
		wx.Panel.__init__(self, parent=parent, id=wx.ID_ANY)
		...
		#					wx.DefaultPosition, wx.Size(500,500),
		#					wx.NO_BORDER | wx.TE_MULTILINE)
		self.splitter = ProportionalSplitter(self, ID_SPLITTER, style= wx.SP_LIVE_UPDATE )
		#self.splitter = wx.SplitterWindow(self, ID_SPLITTER, wx.Point(0, 0),size=(100,100),style=wx.SP_BORDER|wx.SP_LIVE_UPDATE)
		...
		#sys.exit(1)
		self.from_conn=from_conn
		self.qp=SimpleQueryPanel(self.splitter,self, self.from_conn,ignore_change)
		self.rp=SimpleResultsNbPanel(self.splitter,self)
		self.parent=parent
		self.nb=nb
		self.splitter.SplitHorizontally(self.qp, self.rp)
		...
		#self.splitter.Bind(wx.EVT_SPLITTER_SASH_POS_CHANGED, self.OnSashChanged, id=id)
		self.splitter.SetSashPosition(300,True)
...
class SimpleTemplateTabPanel(wx.Panel):
	"""
	This will be the first notebook tab
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent, nb, from_conn,ignore_change=False):
		...
		#sys.exit(1)
		self.from_conn=from_conn
		self.qp=SimpleTemplatePanel(self, self.from_conn,ignore_change)
		#self.rp=SimpleResultsNbPanel(self.splitter,self)
		self.parent=parent
		self.nb=nb
		#self.splitter.SplitHorizontally(self.qp, self.rp)
		self.sizer = wx.BoxSizer(wx.VERTICAL)
		self.sizer.Add(self.qp,1,wx.EXPAND)
		self.SetSizer(self.sizer)
		#text2 = wx.TextCtrl(self, -1, 'Pane 2 - sample text',
		#					wx.DefaultPosition, wx.Size(50,300),
		#					wx.NO_BORDER | wx.TE_MULTILINE)
...
class SimpleTemplatePanel(wx.Panel):
	"""
	This will be the first notebook tab
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent,from_conn,ignore_change=False):
		""""""
		self.ID=parent.ID
		wx.Panel.__init__(self, parent=parent, id=wx.ID_ANY)
		...
		b = wx.Button(self, 10, "Parse to SQL")
		self.Bind(wx.EVT_BUTTON, self.OnClick, b)
		b.SetDefault()
		b.SetSize(b.GetBestSize())
		self.parent=parent
		...
		#sizer.Add(txtOne, 0, wx.ALL, 5)
		#sizer.Add(txtTwo, 0, wx.ALL, 5)
		sizer.Add(bsizer, 0, wx.ALL|wx.EXPAND|wx.RIGHT, 1)
		print 'qp parent id=', parent.Id
		self.editor = CodeEditor(self, abs(parent.Id),readonly=False)
		#self.editor.ignore_change=ignore_change
		self.page = self.onWidgetSetup(self.editor,
								 wx.EVT_KEY_DOWN, self.onTextKeyEvent,
								 sizer)
	...
	def OnClick(self, event):
		#open SQL tab
		text = self.page.GetText()
		#Publisher().sendMessage( "parse_template_to_sql", (self.ID,text) )
		send("parse_template_to_sql", (self.ID,text) )
	...
	def onTextKeyEvent(self, event):
		...
		controlDown = event.CmdDown()
		if not controlDown:
			#Publisher().sendMessage( "disable_template_save", (self.ID) )
			send( "disable_template_save", (self.ID) )
		event.Skip()
...
class SQLPanel(wx.Panel):
	...
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent, log, source,query_in=None):
		""""""
		wx.Panel.__init__(self, parent=parent, id=wx.ID_ANY)
		...
		self.source=source
		self.AddNew('Query', self.query_in)
		if 0:
			if file_to_open:
				if os.path.isfile(file_to_open):
					f = open(file_to_open, 'r')			
					#self.dlg.panel.config.SetValue(f.read())
					self.AddNew(os.path.basename(file_to_open), f.read())
				else:
					print 'Cannot read file %s' % file_to_open
			else:
				for p in range(3):
		...
		if 0:
			Publisher().subscribe(self.AddDescribe, "add_describe")
			
			Publisher().subscribe(self.AddExplainSQL, "add_explain_sql")
			
			Publisher().subscribe(self.reExecSQL, "re_exec_sql")
			Publisher().subscribe(self.SetTextChanged2, "star_tab_name")
	...
	def AddNew(self, label='', value='', ignore_change=False):
		...
		page = TabPanel(self.nb,self.source,ignore_change)
		#print(dir(page))
...
class SimpleSQLPanel(wx.Panel):
	"""
	This will be the first notebook tab
	"""
	#----------------------------------------------------------------------
	def __init__(self, parent, log, from_conn,item, title):
		""""""
		wx.Panel.__init__(self, parent=parent, id=wx.ID_ANY)
		self.parent=parent
		self.ID=parent.ID
		# create the AuiNotebook instance
		#print dir(fnb)
		self.nb = fnb.FlatNotebook(self, -1,size=(700,500), agwStyle=fnb.FNB_NO_X_BUTTON )
		#self.nb.SetTabAreaColour(wx.Color(212,212,212))
		#print dir(self)
		#self.SetBackgroundColour(wx.Color(212,212,212))
		...
		self.from_conn=from_conn
		self.item=item
		self.query_in=item.GetText() #.strip('Query =').strip('"')
		if 0:
			if file_to_open:
				if os.path.isfile(file_to_open):
					f = open(file_to_open, 'r')			
			...
			#print '>>>>>>>>>>>',self.query_in
			self.AddNewTemplate('Template', self.query_in)
			#self.AddNewSql(title, self.query_in)
		
		sizer = wx.BoxSizer(wx.VERTICAL)
		sizer.Add(self.nb, 1, wx.EXPAND)
		sizer.Add((5,5), 0, wx.EXPAND)
		#self.btn_close = wx.Button(self, ID_EXIT, "Close", size=(50,20))
		self.ID_SAVE_AND_EXIT=wx.NewId()
		self.btn_save = wx.Button(self, wx.ID_EXIT, "Save and Close", size=(-1,-1))
		#self.btn_exit = wx.Button(self, self.ID_SAVE_AND_EXIT, "Exit", size=(-1,-1))
		#self.btn_save.Enable(False)
		btn_sizer = wx.BoxSizer(wx.HORIZONTAL)
		
		
		btn_sizer.Add((10,10), 1, wx.EXPAND)
		#btn_sizer.Add(self.btn_save, 0, wx.LEFT)
		#btn_sizer.Add((20,10), 0, wx.EXPAND)
		btn_sizer.Add(self.btn_save, 0, wx.RIGHT)
		#btn_sizer.Add(self.btn_exit, 0, wx.RIGHT)
		...
		#self.Bind(wx.EVT_BUTTON, self.OnExit, id=self.ID_SAVE_AND_EXIT)
		#self.gen_bind(wx.EVT_BUTTON,self.btn_save, self.OnExit,(self.list.current_list))
		self.Bind(wx.aui.EVT_AUINOTEBOOK_PAGE_CLOSED, self.onTabClose, self.nb)
		if 0:
			Publisher().subscribe(self.AddDescribe, "add_describe")
			Publisher().subscribe(self.AddExplainSQL, "add_explain_sql")
			...
			Publisher().subscribe(self.reExecSQL, "re_exec_sql")
			Publisher().subscribe(self.SetTextChanged2, "star_tab_name")
			Publisher().subscribe(self.ParseTemplateToSql, "parse_template_to_sql")
			Publisher().subscribe(self.DisableSave, "disable_template_save")
			Publisher().subscribe(self.DisableSave, "enable_template_save")
		sub(self.AddDescribe, "add_describe")
		sub(self.AddExplainSQL, "add_explain_sql")
		sub(self.reExecSQL, "re_exec_sql")
		sub(self.SetTextChanged2, "star_tab_name")
		sub(self.__ParseTemplateToSql, "parse_template_to_sql")
		sub(self.__EnableSave, "disable_template_save")
		sub(self.__DisableSave, "enable_template_save")
	def gen_bind(self, type, instance, handler, *args, **kwargs):
		self.Bind(type, lambda event: handler(event, *args, **kwargs), instance)
	def __DisableSave(self, data, extra1, extra2=None):
		(ID) = data
		if self.ID==ID:
			self.btn_save.Enable(False)
	def __EnableSave(self, data, extra1, extra2=None):
		(ID) = data
		if self.ID==ID:
			self.btn_save.Enable(True)		
	def DisableSave(self, evt):
		(ID) = evt.data
		if self.ID==ID:
			self.btn_save.Enable(False)
	def EnableSave(self, evt):
		...
		if self.ID==ID:
			self.btn_save.Enable(True)
	def ParseTemplateToSql_del(self, evt):
		(ID,text) = evt.data
		if self.ID==ID:
			self.AddNewSql('Sql', text)
	def __ParseTemplateToSql(self, data, extra1, extra2=None):
		(ID,text) = data
		if self.ID==ID:
			self.AddNewSql('Sql', text)
	...
	def OnExit(self,e):
		...
		print 'in OnExit'
		#save file here
		print e.Id,self.ID_SAVE_AND_EXIT
		if not e.Id==wx.ID_EXIT:			
			self.saveChanges()
			e.Skip()
		else:
			#pprint(dir(self.parent))
			self.parent.Close()
			self.parent.Destroy()
	...
	def saveChanges(self):
		...
		#self.nb.SetPageText(selected_num,'test')
		selected.UnChange()
		pprint(dir(self.item))
	...
	def AddNewSql(self, label='', value='', ignore_change=False):
		if 0 and self.nb.PageCount:
			self.order.append(self.nb.GetSelection())
		...
		page = SimpleTabPanel(self,self.nb,self.from_conn,ignore_change)
		#print(dir(page))
		print 'page.Id= ', page.Id
		page.file_name=label
		page.saved=False
		...
		#print self.nb.PageCount
		#print(dir(self.nb))
		#print self.nb.getPage
		#print dir(self.nb.getPage)
		...
		print 'ending ' , page.qp.editor.ignore_change
		page.qp.editor.ignore_change=True
	def AddNewTemplate(self, label='', value='', ignore_change=False):
		if 0 and self.nb.PageCount:
			self.order.append(self.nb.GetSelection())
		...
		page = SimpleTemplateTabPanel(self,self.nb,self.from_conn,ignore_change)
		#print(dir(page))
		print 'page.Id= ', page.Id
		page.file_name=label
		page.saved=False
...
class Main(wx.Frame):
	def __init__(self, parent, log):
		wx.Frame.__init__(self, parent, -1, "Taco for Netezza", size=(840,680))
...
class QueryEditor(wx.MiniFrame):
	def __init__(self, parent, log, title, from_conn, item):
		#wx.Frame.__init__(self, parent, -1, title, size=(840,680))
		wx.MiniFrame.__init__(self, parent, -1, title,  pos=wx.DefaultPosition, size=(940,780),  style =wx.CAPTION|wx.SYSTEM_MENU |wx.CLOSE_BOX| wx.FRAME_FLOAT_ON_PARENT|wx.THICK_FRAME)
		self.ID=parent.ID
		self.sql_panel = SimpleSQLPanel(self, log, from_conn,item, 'Template')
		#self._mgr = AUIManager(self)
		self.finddata = wx.FindReplaceData()
		self.finddata.SetFlags(wx.FR_DOWN)
		#self.firstTime = True
	...
	def _BuildMenuBar(self):
		...
		if False:
			...
			self.SetAcceleratorTable(aTable)
	def _OnUpdateFindItems(self, evt):
		evt.Enable(self.finddlg == None)			
	def _create_menu_bar(self):
		...
		#accel_tbl = wx.AcceleratorTable([(wx.ACCEL_CTRL,  ord('Q'), randomId )])
		self.SetAcceleratorTable(accel_tbl)	
	def _OnOpenShellWindow(self, evt):
		if self.shell:
			# if it already exists then just make sure it's visible
			s = self.shell
			if s.IsIconized():
			...
			def CloseShell(evt):
				...
				evt.Skip()
			self.Bind(wx.EVT_CLOSE, CloseShell)		
	def _OnOpenWidgetInspector(self, evt):
		# Activate the widget inspection tool
		from wx.lib.inspection import InspectionTool
		if not InspectionTool().initialized:
			InspectionTool().Init()
	...
	def _OnDemoMenu(self, event):
		...
		if selectedDemo:
			self.tree.SelectItem(selectedDemo)
			self.tree.EnsureVisible(selectedDemo)
	def _OnToggleRedirect(self, event):
		app = wx.GetApp()
		if event.Checked():
			app.RedirectStdio()
			print "Print statements and other standard output will now be directed to this window."
