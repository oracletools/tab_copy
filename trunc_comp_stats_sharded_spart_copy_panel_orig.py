...
class ExecQcThread(Thread):
	...
	def run(self):
		...
		start_t= datetime.datetime.now() #.strftime("%y%m%d_%H%M%S")
		...
		if err:	
			if  not self.is_trial:
				...
				Publisher().sendMessage( "sp_copy_done",  (status,err,self.run_id,start_t,end_t) )
			else:
			...
			if  not self.is_trial:
				...
				end_t= datetime.datetime.now() #.strftime("%y%m%d_%H%M%S")
				Publisher().sendMessage( "sp_copy_done",  (out,err,self.run_id, start_t,end_t) )
			else:
...
class ShardedSubPartitionCopyTreeList(HTL.HyperTreeList): #,TextEditMixin):
	def __init__(self, parent, form, id=wx.ID_ANY, pos=wx.DefaultPosition,
		...
		print self.source
		self.ppl_name= '%s "%s"' %  ( tmpl_name, '%s.%s.%s.%s' % tuple(self.source))
		...
		self.SetPyData(self.root,wx.TreeItemData(('etldataflow',{'name':'%s_%s' %  ( tmpl_name, '%s.%s.%s.%s' % tuple(self.source))})))
		...
		if 1:
			...
			worker_ = self.AppendItem(pipeline, 'WORKER "%s"' % self.source[-1])
				...
				if n.nodeType==worker.ELEMENT_NODE:
					...
					for n in self.util_node.getElementsByTagName("param"):
						...
						if n.getAttribute('name')in ('TO_SCHEMA','SCHEMA_NAME'):
							value=self.target[0]
						if n.getAttribute('name')in ('SUBPARTITION'):
							value=self.target[3]							
...
class PCD_Configuration(wx.Panel):
	...
	def __init__(self, parent,style,data,pos,spath,tpath, sides):
		...
		self.tree = ShardedSubPartitionCopyTreeList(self.splitter,self, -1 #, #style =
										#wx.TR_DEFAULT_STYLE
										#| wx.TR_HAS_BUTTONS
										#| wx.TR_TWIST_BUTTONS
										#| wx.TR_ROW_LINES
		...
		if 1:
			...
			self.splitter.AppendWindow(self.tree,(self.frame.msize[1]/5)*4)				
...
class ShardedSubPartitionCopyPanel(wx.Panel):
	def __init__(self, parent,pos,drag_pos, drop_pos, dd_data, sides ):
		...
		Publisher().subscribe(self.onCopyDone, "sp_copy_done")
	...
	def OnStart(self,e):
		...
		Publisher().sendMessage( 'append_transfer_log', (((self.ID,self.run_id),self.source[-1],'Sub-Partition','/'.join(self.cfrom.split('/')[3:]),'/'.join(self.cto.split('/')[3:]),'n/a','Started','n/a','n/a'),))
		if 1:
