...
class ExecQcThread(Thread):
	...
	def run(self):
		...
		if err:	
			...
			if  not self.is_trial:
				Publisher().sendMessage( "append_log", ('Executing table copy...',self.obj.pos,self.run_id[0]) )
...
class ShardedTableCopyTreeList(HTL.HyperTreeList): #,TextEditMixin):
	def __init__(self, parent, form, id=wx.ID_ANY, pos=wx.DefaultPosition,
		...
		print self.source
		print self.target
		self.ppl_name= tmpl_name+'_%s_%s' % tuple(self.source)
		...
		if 1:
			...
			if 0:
				workflow = self.AppendItem(pipeline, 'SYNC')
				...
				self.SetPyData(workflow,wx.TreeItemData((0,1,'SYNC')))
			...
			worker_ = self.AppendItem(pipeline, 'WORKER "%s.%s"' % tuple(self.source))
			...
			for n in worker.childNodes:
				if n.nodeType==worker.ELEMENT_NODE:
					...
					self.SetPyData(task,wx.TreeItemData((exec_node.nodeName,{'util_node_name':self.util_node.nodeName,'util_method':util_method})))
...
class ShardedTableCopyPanel(wx.Panel):
	def __init__(self, parent,pos,drag_pos, drop_pos, dd_data, table_name, sides  ):
		self.ID=wx.NewId()
		...
		#sys.exit(0)
		self.config_panel=STCD_Configuration(self,style=wx.TAB_TRAVERSAL|wx.CLIP_CHILDREN, data=self.data,pos=pos, spath=self.cfrom, tpath=self.cto, sides=sides)
	...
	def OnStart(self,e):
		...
		Publisher().sendMessage( 'append_transfer_log', (((self.ID,self.run_id),self.source[-1],'Table','/'.join(self.cfrom.split('/')[3:]),'/'.join(self.cto.split('/')[3:]),'n/a','Started','n/a','n/a'),))
