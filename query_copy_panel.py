...
def log(msg, pos=None, panel_id=None):
	#wx.LogMessage(msg)
	Publisher().sendMessage( "append_log", (msg,pos,panel_id) )
	...
	def run(self):
		...
		if err:	
			if  not self.is_trial:
				...
				Publisher().sendMessage( "query_copy_done", (status,err,self.run_id,start_t,end_t) )
			else:
			...
			if  not self.is_trial:
				...
				end_t= datetime.datetime.now()
				Publisher().sendMessage( "query_copy_done", (out,err,self.run_id, start_t,end_t)  )
			else:
...
class QueryCopyTreeList(HTL.HyperTreeList): #,TextEditMixin):
	def __init__(self, parent, form, id=wx.ID_ANY, pos=wx.DefaultPosition,
		...
		self.ppl_name= 'QUERY_COPY_%s' % form.ts
		prof=def_w_prof
		...
		if 1:
			...
			self.source=(schema_to,t_name)
			worker.setAttribute('name','%s.%s' % (schema_to,t_name) )
			#param=worker.getElementsByTagName("param")[0]
			for n in worker.getElementsByTagName("param") :
				print n.getAttribute('name')
			#param= [n for n in worker.getElementsByTagName("param") if n.getAttribute('name')=='SUBPARTITION'][0]
			...
			worker_ = self.AppendItem(pipeline, 'WORKER "%s.%s"' % (schema_to,t_name))
			#worker_ = self.AppendItem(pipeline, 'WORKER')
			...
			self.SetPyData(worker_,wx.TreeItemData(('worker',{'name':'%s.%s' % (schema_to,t_name)})))				
			for n in worker.childNodes:
				if n.nodeType==worker.ELEMENT_NODE:
					...
					cd= self.createQuery(form.data,form.spath) #[n for n in self.util_node.getElementsByTagName("sql_template")[0].childNodes if n.nodeType==worker.CDATA_SECTION_NODE][0]
					cdata = self.AppendItem(task, '"%s"' % cd)
					
					#self.tree.SetItemText(query, '%s...' % q[:15], 1)
					
					#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
					#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
					self.SetPyData(cdata,wx.TreeItemData(('cdata',{'tag':'sql_template','cdata':cd})))
					#self.Expand(task)
			...
			if 0:		
				...
				self.SetItemImage(worker_, fldropenidx, which = wx.TreeItemIcon_Expanded)			
				self.SetPyData(worker_,wx.TreeItemData(('worker',{'name':'%s.%s' % (schema_to,t_name)})))	
				task = self.AppendItem(worker_, 'task "Query Copy"')
				#self.tree.SetItemText(wk, 'Template: Query Copy', 1)
				self.SetItemImage(task, fldridx, which = wx.TreeItemIcon_Normal)
				self.SetItemImage(task, fldropenidx, which = wx.TreeItemIcon_Expanded)			
				self.SetPyData(task,wx.TreeItemData(('exec_query_copy',{})))
				...
				for n in tasklet.getElementsByTagName("param"):
					#print 
					child = self.AppendItem(locals, '%s = "%s"' % (n.getAttribute('name'),n.getAttribute('value')))
					#self.tree.SetItemText(child, def_w_prof, idx)
					print n.getAttribute('name'), n.getAttribute('value')
					#self.tree.SetItemText(child, n.getAttribute('value'), 1)
					self.SetPyData(child,wx.TreeItemData(('param',{'name':n.getAttribute('name'),'value':n.getAttribute('value') })))
				...
				self.Expand(locals)
				self.q=self.createQuery(form.data,form.spath)
				query = self.AppendItem(task, 'query = "%s"' % self.q)
				
				#self.tree.SetItemText(query, '%s...' % q[:15], 1)
				
				#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
				#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
				self.SetPyData(query,wx.TreeItemData(('cdata',{'tag':'sql_template','cdata':self.q})))
				...
				for w in r.GetChildren():
					...
					if w_tag in ('worker'):
						...
						for t in w.GetChildren():
							...
							t_utils.setAttribute('method','%QUERY_COPY_METHOD%' )
							if 0:
...
class QCD_Configuration(wx.Panel):
	...
	def __init__(self, parent,style,data,pos,spath,tpath):
		wx.Panel.__init__(self, parent, -1, style=style)
		self.data=data
		self.parent=parent
		...
		if 0:
			if 0:
				self.spath=self.frame.getVarsToPath(parent.pos_from)
				self.tpath=self.frame.getVarsToPath(parent.pos_to)		
			else:
		...
		self.tree = QueryCopyTreeList(self.splitter,self, -1 #, #style =
										#wx.TR_DEFAULT_STYLE
										#| wx.TR_HAS_BUTTONS
										#| wx.TR_TWIST_BUTTONS
										#| wx.TR_ROW_LINES
		...
		if 1:
			...
			#msize=self.frame.splitter.GetSizeTuple()
			msize= self.frame.msplitter.GetSizeTuple() 
			#print fsize
			#sys.exit(1)
			self.splitter.SetMinimumPaneSize(50)
			#self.splitter.SplitHorizontally(self.tree, self.nbe)
			self.splitter.AppendWindow(self.tree,(msize[1]/3)*2)				
...
class QueryCopyPanel(wx.Panel):
	def __init__(self, parent,pos,drag_pos, drop_pos, dd_data, sides ):
		...
		if 1:
			...
			#print parent.pos_from,self.cfrom
			#print parent.pos_to,self.cto
			#sys.exit(0)
		else:
		...
		self.config_panel=QCD_Configuration(self,style=wx.TAB_TRAVERSAL|wx.CLIP_CHILDREN, data=self.data,pos=pos, spath=self.cfrom, tpath=self.cto)
		...
		Publisher().subscribe(self.onCopyDone, "query_copy_done")
	...
	def getTargetTable(self):
		return self.config_panel.tree.to_table[1]
	def OnStart(self,e):
		if 0:
			print self.pos_to
		...
		Publisher().sendMessage( 'append_transfer_log', (((self.ID,self.run_id),'Query','Query','/'.join(self.cfrom.split('/')[3:]),'/'.join(('%s/%s' % (self.cto, self.getTargetTable())).split('/')[3:]),'n/a','Started','n/a','n/a'),))
