...
class QueryCopyTreeList(HTL.HyperTreeList): #,TextEditMixin):
	def __init__(self, parent, form, id=wx.ID_ANY, pos=wx.DefaultPosition,
		...
		if 1:
			...
			self.SetPyData(runat,wx.TreeItemData((0,1,'runat')))
			runat_server = self.AppendItem(runat, 'server = "%s"' % tc_host[tc_srv][2])
			#self.tree.SetItemText(runat_server, tc_host[tc_srv][2], 1)
			self.SetPyData(runat_server,wx.TreeItemData((0,1,'server')))
			runat_home = self.AppendItem(runat, 'home = "%s"' % tc_loc[tc_srv][tc_home][0])
			#self.tree.SetItemText(runat_home, tc_loc[tc_srv][tc_home][0], 1)
			self.SetPyData(runat_home,wx.TreeItemData((0,1,'home')))
			runat_user = self.AppendItem(runat, 'user = "ab95022"')
			#self.tree.SetItemText(runat_user, 'ab95022', 1) #tc_host[tc_srv][2]
			self.SetPyData(runat_user,wx.TreeItemData((0,1,'user')))
			...
			globals = self.AppendItem(self.root, 'globals')
			#self.tree.SetItemImage(globals, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(globals, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.SetPyData(globals,wx.TreeItemData((0,1,'globals')))
			p=0
			for n in gl.getElementsByTagName("param"):
				
				#for i in range(10):
				
				child = self.AppendItem(globals, '%s = "%s"' %(n.getAttribute('name'),n.getAttribute('value')))
				...
				self.SetPyData(child,wx.TreeItemData((1,p,n.getAttribute('value'))))
				p +=1
			self.Expand(globals)
			...
			self.SetPyData(pipeline,wx.TreeItemData((0,2,'PIPELINE')))
			if 0:
			...
			self.SetPyData(callbefore,wx.TreeItemData((0,0,'CallBefore')))
			worker_stab=doc.getElementsByTagName("worker")[0].toxml()
			worker=xml.dom.minidom.parseString(worker_stab).getElementsByTagName("worker")[0]
			#pprint(dir(worker))
			...
			self.SetPyData(worker_,wx.TreeItemData((1,0,'worker')))
			...
			self.SetPyData(task,wx.TreeItemData((1,0,'task')))
			
			locals = self.AppendItem(task, 'locals')
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.SetPyData(locals,wx.TreeItemData((0,0,'locals')))
			p=0
			for n in tasklet.getElementsByTagName("param"):
				...
				p +=1
			self.Expand(locals)
			q=self.createQuery(form.data,form.spath)
			query = self.AppendItem(task, 'query = "%s"' % q)
			
			#self.tree.SetItemText(query, '%s...' % q[:15], 1)
			
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.SetPyData(query,wx.TreeItemData((0,0,'query')))
			self.Expand(task)
			self.Expand(worker_)
			...
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.SetPyData(callafter,wx.TreeItemData((0,2,'CallAfter')))			
	...
	def __init__(self, parent,style,data):
		...
		if 1:
			...
			if 1:
				...
				self.nbe.AddPage(self.query, self.tree.to_table[idx])
...
class QueryCopyPanel(wx.Panel):
	...
	def OnDeployXml(self,e):
		...
		if 1:
			...
			self.table_to[ppl_row[0]]=(conf.tree.ppl_name, conf.to_table,(conf.query.GetText(),), ppl_row, w_row)
			pprint(self.table_to)
			#sys.exit(1)
			if 0:
				for i in range(len(self.data)):
					...
					self.table_to[row[0]]=(self.config_panel.tree.ppl_name, self.config_panel.to_table,row)
