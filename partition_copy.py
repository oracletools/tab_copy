...
_flags['Truncate Part-n']=	(_ON,		_OFF,		_OFF,	_OFF,		_OFF,	_OFF,		_OFF)
...
def createPcPipelineConfig(cfrom,cto,config_file):
	#create config file
...
def createPcPipelineWorker(cfrom,cto,config, worker_file,tables,  _tcmode):
	#create config file
	max_shards=20
	print appLoc
	db_from =cfrom.split('/')[3]
	db_to =cto.split('/')[3]
	schema_from =cfrom.split('/')[-2]
	schema_to =cto.split('/')[-2]
	#print 
	#sys.exit(1)
	tmpl_loc=os.path.join(appLoc,'xml_templates')
	template_name='partition_copy.xml'
	...
	base.setAttribute("name","PARTITION_COPY")
	...
	for key, tabs in tables.items():
		(tab,tab_to,partition,item) =tabs
		(partition_to,copy_profile, trunc,compress,stats, rebuildIdx,shards, createt, copyd) = item
		#print tabs
		#sys.exit(1)
		...
		##
		param.setAttribute('value',partition)
		...
		if 	compress=='ON':
			...
			tmpl_loc=os.path.join(appLoc,'xml_templates')
			template_name='compress_partition.xml'
			tmpl_file_loc= os.path.join(tmpl_loc,template_name)
			...
			part_name=[n for n in comp_tasklet.getElementsByTagName("param") if n.getAttribute('name')=='PARTITION'][0]
			part_name.setAttribute('value',partition)
			worker.insertBefore(comp_tasklet,exec_copy)
		print stats
		if stats=='ON':
			assert stats in ('ON', 'OFF'), 'Stats should be "ON" or "OFF".'
			tmpl_loc=os.path.join(appLoc,'xml_templates')
			template_name='gather_partition_stats.xml'
			tmpl_file_loc= os.path.join(tmpl_loc,template_name)
			...
			_schema_name.setAttribute('value',schema_to)
			part_name=[n for n in stats_tasklet.getElementsByTagName("param") if n.getAttribute('name')=='PARTITION'][0]
			part_name.setAttribute('value',partition)
...
class ExecPcThread(Thread):
	...
	def run(self):
		...
		if 1:		
			...
			(local_path,remote_path, config_file)= createPcPipelineConfig(self.cfrom,self.cto,self.xml_config)
			...
			(out_dir,worker_file,remote_loc)= createPcPipelineWorker(self.cfrom,self.cto,self.xml_config,self.xml_worker, self.table_to, self._tcmode )
...
class PcDeployXmlLogPanel(wx.Panel):
...
class PCD_Configuration(wx.Panel):
	...
	def __init__(self, parent, frame,style,data):
		...
		if len(self.data)>1:
			suffix='s'
		label = wx.StaticText(self, -1, "Copy %d partition%s." % (len(self.data),suffix))
		label.SetHelpText('Number of partitions to copy. \nPless "Cancel" button to do do modifications.')
		...
		label.SetHelpText("Partition copy source table.")
		...
		#text.SetLabel()
		self.from_loc.SetHelpText("Partition copy Source URL")
		...
		self.from_table = wx.TextCtrl(self, -1, self.parentFrame.getVarsToPath(parent.pos_from).split('/')[-1], size=(200,-1))
		self.from_table.Enable(False)
		#text.SetLabel()
		self.from_table.SetHelpText("Partition copy Source Table")
		...
		label.SetHelpText("Partition copy target table.")
		...
		self.to_loc.SetHelpText("Partition copy TARGET table")
		box.Add(self.to_loc, 1, wx.ALIGN_CENTRE|wx.ALL, 0)
		label = wx.StaticText(self, -1, "To Table:",size=(60,-1))
		label.SetHelpText("Partition copy target table.")
		box.Add((10,5), 0, wx.LEFT, 0)
		box.Add(label, 0, wx.LEFT, 0)
		self.to_table = wx.TextCtrl(self, -1, self.parentFrame.getVarsToPath(parent.pos_from).split('/')[-1], size=(200,-1))
		self.to_table.Enable(True)
		#text.SetLabel()
		self.to_table.SetHelpText("Partition copy Target Table")
...
class PartitionCopyDialog(wx.Dialog):
	def __init__(
		...
		# contents
		sizer = wx.BoxSizer(wx.VERTICAL)
		self.config_panel=PCD_Configuration(self,parent,style=wx.TAB_TRAVERSAL|wx.CLIP_CHILDREN, data=self.data)
		self.nb.AddPage(self.config_panel, 'Configuration')
		self.code_panel=PcCodePanel(self, style=wx.TAB_TRAVERSAL|wx.CLIP_CHILDREN)
		self.nb.AddPage(self.code_panel, 'Code')
		self.deploy_panel=PcDeployXmlLogPanel(self, style=wx.TAB_TRAVERSAL|wx.CLIP_CHILDREN)
	...
	def OnDeployXml(self,e):
		...
		if 1:
			...
			Publisher().sendMessage( "deploy_pc_xml", (self.pos_from, self.pos_to, self.data,self.table_to, self.config_panel._tcmode) )
	...
	def OnStart(self,e):
		...
		if 1:
			#table_to=dlg.table_to	
			self.btn_start.Enable(False)
			Publisher().sendMessage( "start_partition_copy", (self.pos_from, self.pos_to, self.data,self.table_to, self.config_panel._tcmode) )
