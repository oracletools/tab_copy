...
class QCD_Configuration(wx.Panel):
	...
	def __init__(self, parent,style,data):
		...
		if 1:
			...
			max_shards=20
			print appLoc
			db_from =spath.split('/')[3]
			db_to =tpath.split('/')[3]
			schema_from =spath.split('/')[4]
			schema_to =tpath.split('/')[4]
			print spath
			print tpath
			#sys.exit(1)
			tmpl_loc=os.path.join(appLoc,'xml_templates')
			...
			doc_to.appendChild(base)
			gl=doc.getElementsByTagName("globals")[0]
			runat = self.tree.AppendItem(self.tree.root, 'runat')
			...
			self.tree.SetPyData(runat,wx.TreeItemData((0,1,'runat')))
			runat_server = self.tree.AppendItem(runat, 'server')
			self.tree.SetItemText(runat_server, tc_host[tc_srv][2], 1)
			self.tree.SetPyData(runat_server,wx.TreeItemData((0,1,'server')))
			runat_home = self.tree.AppendItem(runat, 'home')
			self.tree.SetItemText(runat_home, tc_loc[tc_srv][tc_home][0], 1)
			self.tree.SetPyData(runat_home,wx.TreeItemData((0,1,'home')))
			runat_user = self.tree.AppendItem(runat, 'user')
			self.tree.SetItemText(runat_user, 'ab95022', 1) #tc_host[tc_srv][2]
			self.tree.SetPyData(runat_user,wx.TreeItemData((0,1,'user')))
			...
			globals = self.tree.AppendItem(self.tree.root, 'globals')
			self.tree.SetItemImage(globals, fldridx, which = wx.TreeItemIcon_Normal)
			self.tree.SetItemImage(globals, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(globals,wx.TreeItemData((0,1,'globals')))
			p=0
			for n in gl.getElementsByTagName("param"):
				
				#for i in range(10):
				
				child = self.tree.AppendItem(globals, n.getAttribute('name'))
				...
				self.tree.SetPyData(child,wx.TreeItemData((1,p,n.getAttribute('value'))))
				p +=1
			self.tree.Expand(globals)
			pipeline = self.tree.AppendItem(self.tree.root, 'PIPELINE')
			self.tree.SetItemImage(pipeline, fldridx, which = wx.TreeItemIcon_Normal)
			self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(pipeline,wx.TreeItemData((0,2,'PIPELINE')))
			if 0:
				block = self.tree.AppendItem(pipeline, 'BLOCK')
				#self.tree.SetItemText(block, def_ppl_prof, 1)
				self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
				self.tree.SetItemImage(block, fldropenidx, which = wx.TreeItemIcon_Expanded)			
				self.tree.SetPyData(block,wx.TreeItemData((2,0,'BLOCK')))
				
			workflow = self.tree.AppendItem(pipeline, 'ASYNC')
			self.tree.SetItemText(workflow, 'Parallel flow block', 1)
			self.tree.SetItemImage(workflow, fldridx, which = wx.TreeItemIcon_Normal)
			self.tree.SetItemImage(workflow, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(workflow,wx.TreeItemData((0,1,'ASYNC')))
			
			callbefore = self.tree.AppendItem(workflow, 'CallBefore')
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(callbefore,wx.TreeItemData((0,0,'CallBefore')))
			worker_stab=doc.getElementsByTagName("worker")[0].toxml()
			worker=xml.dom.minidom.parseString(worker_stab).getElementsByTagName("worker")[0]
			#pprint(dir(worker))
			...
			wk = self.tree.AppendItem(workflow, 'worker')
			self.tree.SetItemText(wk, 'Template: Query Copy', 1)
			self.tree.SetItemImage(wk, fldridx, which = wx.TreeItemIcon_Normal)
			self.tree.SetItemImage(wk, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(wk,wx.TreeItemData((1,0,'worker')))
			locals = self.tree.AppendItem(wk, 'locals')
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(locals,wx.TreeItemData((0,0,'locals')))
			p=0
			for n in tasklet.getElementsByTagName("param"):
				#print 
				child = self.tree.AppendItem(locals, n.getAttribute('name'))
				#self.tree.SetItemText(child, def_w_prof, idx)
				print n.getAttribute('name'), n.getAttribute('value')
				self.tree.SetItemText(child, n.getAttribute('value'), 1)
				self.tree.SetPyData(child,wx.TreeItemData((1,p,n.getAttribute('value'))))
				p +=1
			query = self.tree.AppendItem(wk, 'query')
			self.tree.SetItemText(query, 'SELECT * FROM DUAL', 1)
			
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(query,wx.TreeItemData((0,0,'query')))
			self.tree.Expand(wk)
			self.tree.Expand(workflow)
			callafter = self.tree.AppendItem(workflow, 'CallAfter')
			#self.tree.SetItemImage(block, fldridx, which = wx.TreeItemIcon_Normal)
			#self.tree.SetItemImage(pipeline, fldropenidx, which = wx.TreeItemIcon_Expanded)			
			self.tree.SetPyData(callafter,wx.TreeItemData((0,2,'CallAfter')))			
			
			self.tree.Expand(workflow)
			self.tree.Expand(pipeline)
			if 0:
				flowt= [n for n in gl.getElementsByTagName("param") if n.getAttribute('name')=='FLOW_TYPE'][0]
				flowt.setAttribute('value',_tcmode)
				fromdb= [n for n in gl.getElementsByTagName("param") if n.getAttribute('name')=='FROM_DB'][0]
